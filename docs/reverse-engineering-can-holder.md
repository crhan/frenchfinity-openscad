...
>
> 2026-06-29（**第二轮，本次**）用户按图形复核发现仍「少了一些倒圆角」。逐层测量后发现差的
...
- 文字 5 行刻在**斜前面**：`rotate([90-a,0,0]) rotate([0,180,0])` —— `90-a` 贴斜面、
  `yrot(180)` 是 +Y 外面的翻转（同 `labelFace` 对 `y_face>0` 的处理），否则**镜像**（踩过）。
