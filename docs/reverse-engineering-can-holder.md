...
顶面 ⊥ bore 轴                                          # 构造保证
H  = base + (ci+rim)·cos(a) + (r+leadin)·sin(a)        # 顶平台前沿 = 开口后缘(开口完整)
     其中 r=(cd+clearance)/2, rim=4, leadin=3
```

## 验证（mine vs 1.0，dx/dy/dz，mm）

- cd23/pl18/ci55/p8 **a20**：`39.00/66.35/75.14` vs `39.00/68.16/73.91` —— dy −1.8、dz +1.2
- cd10/pl16/ci55/p10 **a20**：`30.00/55.38/72.32` vs `30.00/57.70/72.71` —— dy −2.3、dz −0.4
- cd40/pl30/ci70/p10 **a30**：`60.00/100.53/94.09` vs `60.00/94.84/101.28` —— 大角度大罐偏差较大

`dx` 逐 mm 命中；常用 a≤20 例 dy/dz 在 ±2mm。a30 大罐的高度仍偏（H 公式对角度的
追踪是功能近似，未对全 40 样本完美拟合）。**形状（斜顶/前倾/导入倒角/前+顶倒圆/背面尖角/
开口完整）按图形已对齐 1.0。** 文字回归 `python3 test/test_labels_fit.py`：can_holder 各例通过（66/66）。

> 仍有的小差异（待用户定夺）：① 我方 countersink 漏斗略宽于 1.0;② 后平台略深（~23 vs ~18mm）。
> 二者皆为微调,且 1.0 顶部「平台+开口」的精确参数关系尚未完全反推出（bore floor 与 ⊥屋脊法向
> 相同,数值难分离;靠自渲 PNG 图形对比已足够接近）。
